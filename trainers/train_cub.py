...
from model.c2f_cub import Fine_Align
from model.c2f_cub import Coarse_Align
...
def main(opt, pkl_data):
    ...
    coarse_info = 'coarse_lr:{}, coarse_decay:{}, coarse_iter_num:{}, coarse_way_shot:{}, seed:{}, pkl_data_path:{}'\
        .format(cfg.coarse_lr, cfg.coarse_opt_decay, cfg.coarse_iter_num, [cfg.ways, cfg.shots], cfg.seed, pkl_data.split('/')[-1])
    ...
    coarse_align_model = Coarse_Align(
        args=opt,
        seen_att=seen_att,
    ...
    # optimizer_coarse = t.optim.SGD(
    #     [{'params': coarse_align_model.backbone.parameters(), 'lr': opt.coarse_lr}, {'params': coarse_align_model.V2S.parameters(), 'lr':  opt.coarse_lr}],
    #                         lr=opt.coarse_lr,
    #                         momentum=opt.coarse_momentum,
    #                         weight_decay=opt.coarse_opt_decay)
    ...
    if not opt.transductive:
        if opt.train_stage == 'coarse':
            ...
            new_seen_class_indices_inside_test, new_unseen_class_indices_inside_test, seen_b_classes = \
        ...
        # warm_up_epoch = 60 # 60 # 40
        # epoch_interval = 20 # 20  # 20

        for epoch in range(opt.fine_epoch_num):
            fine_align_model.train()
            if epoch >= cfg.warm_epoch and epoch % cfg.warm_epoch == 0:
            
                # if use pseudo labels to improve the  visual mebeddings, then update the coarse training dataloader and update backbone
...
if __name__ == "__main__":
    ...
    if platform.system() == 'Linux':
        cfg = parse_args(config_file='config/cub_hyp_para.yaml')
        
    if platform.system() == 'Linux':
        main(opt=cfg, pkl_data='CUB_0_data_ck.pkl')
